...
- All environment variables must use `UPPER_SNAKE_CASE` naming (e.g., `GROQ_API_KEY`, `DATABASE_URL`, `VECTOR_STORE_PATH`).
...
Every response generated by the AI (via Groq or any LLM provider) must follow these formatting rules:
...
- **The LLM provider must remain replaceable.** All Groq-specific logic must be isolated behind a single integration module (`groq_service`), so switching providers requires modifying one file, not logic scattered throughout the codebase.
